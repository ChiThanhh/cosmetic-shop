  ...
  return (
    <main className="bg-white text-gray-700">
      ...
      <section  data-aos="fade-down" 
        className="relative bg-pink-100 py-12 bg-cover bg-center"
        style={{ background: `url(${bannerImage})` }}
      >
        <div className="absolute inset-0 bg-black opacity-70"></div>
        <div className="relative max-w-7xl mx-auto px-4 text-center">
          <h1  data-aos="fade-up"  className="text-3xl md:text-5xl font-bold text-pink-100">
            Mỹ phẩm chính hãng, đẹp tự nhiên
          </h1>
          <p   data-aos="fade-down" className="mt-4 text-gray-400 text-lg">
            Cửa hàng mỹ phẩm uy tín – 100% sản phẩm chính hãng, an toàn cho làn
            da bạn.
          </p>
          <a
           data-aos="fade-down" 
            href="/san-pham"
            className="inline-block mt-6 px-6 py-3 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transform hover:scale-105 transition-all duration-300 ease-in-out"
            >
            Mua ngay
          </a>
        </div>
      ...
      <section className="py-12">
        <div className="max-w-7xl mx-auto px-4">
          <h2  data-aos="fade-right"  className="text-2xl font-semibold mb-6 text-pink-600">
            Danh mục nổi bật
          </h2>
          <div  data-aos="fade-up"  className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {["Trang điểm", "Chăm sóc da", "Nước hoa", "Phụ kiện"].map(
              (cat, idx) => (
                <div
                  key={idx}
                  className="bg-pink-50 p-4 rounded-lg text-center hover:shadow-lg"
                >
